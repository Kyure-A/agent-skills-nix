...
- `apps.<system>.skills-install`: Sync bundle to global targets (`$HOME/.agents/skills` for agentskills.io standard, `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills` for Claude Code, `$HOME/.copilot/skills` for GitHub Copilot). Override destinations with `AGENT_SKILLS_DESTS`.
- `apps.<system>.skills-install-local`: Sync bundle to local targets (default `.agents/skills`, `.claude/skills`, `.github/skills` with `copy-tree`). Override root with `AGENT_SKILLS_ROOT`, destinations with `AGENT_SKILLS_LOCAL_DESTS`.
...
Local skills are installed to `.agents/skills`, `.claude/skills`, and `.github/skills` relative to the current working directory (or `AGENT_SKILLS_ROOT` if set). Override destinations via `AGENT_SKILLS_LOCAL_DESTS`.
...
Then run `nix run .#skills-install-local` from your project root to install skills to `.agents/skills`, `.claude/skills`, and `.github/skills`.
